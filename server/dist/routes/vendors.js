...
router.post('/update-missions', async (req, res) => {
    ...
    try {
        ...
        await (0, connection_1.query)(sqlScript);
        // Get updated counts
        const missionCount = await (0, connection_1.query)('SELECT COUNT(*) as count FROM missions');
        const responsibleCount = await (0, connection_1.query)('SELECT COUNT(*) as count FROM responsible');
...
router.post('/add-remaining-parishes', async (req, res) => {
    if (process.env.NODE_ENV === 'production') {
        return res.status(404).json({ success: false, error: 'Not found' });
    }
    try {
